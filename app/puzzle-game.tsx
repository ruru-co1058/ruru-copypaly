  ...
  <section className="workspace">
   ...
   <section className="game-card">
    ...
    <div className="game-area">
     ...
     <><div className={`card-grid ${status==='won'?'is-frozen':''}`} style={{'--columns':pictures.length<=3?3:pictures.length<=6?4:6} as React.CSSProperties}>{cards.map((card,index)=>{const picture=pictures.find(p=>p.id===card.pictureId);const shown=open.includes(index)||card.matched;return <button key={card.key} className={`memory-card ${shown?'is-flipped':''} ${card.matched?'is-matched':''}`} disabled={card.matched||status==='won'} onClick={()=>flip(index)} aria-label={shown?`第 ${index+1} 張牌，已翻開`:`翻開第 ${index+1} 張牌`}><span className="card-inner"><span className="card-back"><span>?</span></span><span className="card-front">{picture&&<img src={picture.url} alt="配對圖片"/>}{card.matched&&<i><Check/></i>}</span></span></button>})}</div><p className="game-tip">翻開兩張牌。配對正確的圖片會留在原位顯示。</p>{status==='won'&&<div className="win-panel" role="status" aria-live="assertive"><div className="victory-star">★</div><h2>全部配對成功！</h2><span className="finish-label">完成時間</span><strong className="finish-time">{clock(seconds)}</strong><p>共翻牌 <strong>{moves}</strong> 次</p><button onClick={start}><RotateCcw/>再玩一次</button></div>}</>}
    </div>
   </section>
  </section>
 </main>
