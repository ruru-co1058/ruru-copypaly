...
export default function PictureMatchGame({autoStartSignal=0,player=0}:{autoStartSignal?:number;player?:number}){
 ...
 useEffect(()=>{if(player===0)window.dispatchEvent(new CustomEvent('shared-match-pictures',{detail:pictures}))},[pictures,player]);
 useEffect(()=>{if(player!==1)return;const sync=(event:Event)=>setPictures((event as CustomEvent<Picture[]>).detail);window.addEventListener('shared-match-pictures',sync);return()=>window.removeEventListener('shared-match-pictures',sync)},[player]);
 ...
 useEffect(()=>{if(autoStartSignal>0)start()},[autoStartSignal]);
