...
export default function PictureMatchGame(){
 ...
 const flip=(index:number)=>{if(status!=='playing'||locked||open.includes(index)||cards[index].matched)return;const next=[...open,index];setOpen(next);if(next.length<2)return;setMoves(value=>value+1);setLocked(true);const [first,second]=next;if(cards[first].pictureId===cards[second].pictureId){window.setTimeout(()=>{setCards(current=>current.map((card,i)=>i===first||i===second?{...card,matched:true}:card));setOpen([]);setLocked(false);if(cards.filter(card=>card.matched).length+2===cards.length)setStatus('won')},450)}else{window.setTimeout(()=>{setOpen([]);setLocked(false)},900)}};
  ...
  <section className="workspace">
   ...
   <section className="game-card">
    ...
    <div className="game-area">
     ...
     <><div className="card-grid" style={{'--columns':pictures.length<=3?3:pictures.length<=6?4:6} as React.CSSProperties}>{cards.map((card,index)=>{const picture=pictures.find(p=>p.id===card.pictureId);const shown=open.includes(index)||card.matched;return <button key={card.key} className={`memory-card ${shown?'is-flipped':''} ${card.matched?'is-matched':''}`} disabled={card.matched} onClick={()=>flip(index)} aria-label={shown?`第 ${index+1} 張牌，已翻開`:`翻開第 ${index+1} 張牌`}><span className="card-inner"><span className="card-back"><span>?</span></span><span className="card-front">{picture&&<img src={picture.url} alt="配對圖片"/>}{card.matched&&<i><Check/></i>}</span></span></button>})}</div><p className="game-tip">翻開兩張牌。配對正確的圖片會留在原位顯示。</p>{status==='won'&&<div className="win-panel"><div>★</div><h2>全部配對成功！</h2><p>用了 <strong>{clock(seconds)}</strong>，翻牌 <strong>{moves}</strong> 次。</p><button onClick={start}><RotateCcw/>再玩一次</button></div>}</>}
    </div>
   </section>
  </section>
 </main>
